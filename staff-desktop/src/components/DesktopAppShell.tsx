...
const SIDEBAR_LINKS = [
  ...
  { path: "/waiter", label: "Tables", icon: "🪑", roles: ["waiter", "delivery", "manager", "admin"] as const },
  ...
  { path: "/settings", label: "Settings", icon: "⚙️", roles: ["cashier", "kitchen", "waiter", "delivery", "manager", "admin"] as const }
];
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-100 dark:bg-slate-950">
      {!hideSidebar ? (
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-4 dark:border-slate-800">
            {!sidebarCollapsed ? (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-brand-teal">Nausheen Staff</p>
                <p className="text-[10px] text-slate-500">Desktop POS</p>
              </div>
            ) : (
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div>
          ...
          <div className="flex items-center gap-3">
            <LiveClock />
            <span className="hidden rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700 lg:inline dark:bg-emerald-900/40 dark:text-emerald-300">
              Shift Open
            </span>
            ...
            {onLogout ? (
              <button
                ...
                className="min-h-[44px] rounded-lg border-2 border-red-300 px-5 py-2 text-sm font-bold text-red-600 hover:bg-red-50 dark:border-red-800 dark:text-red-400"
              >
                Logout
              </button>
            ) : null}
