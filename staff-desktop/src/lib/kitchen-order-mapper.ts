...
import { resolveKitchenQueueStatus, type KitchenOrder } from "@/lib/kds-utils";
...
function resolveKitchenSource(order: StaffOrderRow): KitchenOrder["source"] {
  ...
  if (src === "dine_in" || src === "waiter") return "dine-in";
  return "takeaway";
}
...
function mapItems(order: StaffOrderRow, withExtras: boolean) {
  return order.items.map((it, index) => {
    const extras = withExtras ? formatItemExtras(it) : undefined;
    return {
