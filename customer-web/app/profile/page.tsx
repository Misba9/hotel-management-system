...
function ProfileAuthenticatedPage() {
  ...
  const { orders, loading: ordersLoading, error: ordersError } = useCustomerOrders({ maxDocs: 100 });
        ...
        <div className="min-w-0 flex-1 space-y-6">
              ...
              <div className="space-y-4 lg:col-span-5">
                <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-md dark:border-slate-700 dark:bg-slate-900">
                  ...
                  {ordersError && !ordersLoading ? (
                    <p className="mt-3 text-xs text-amber-800 dark:text-amber-200/90">{ordersError}</p>
                  ) : null}
                  {ordersLoading ? (
                    <QuickStatsSkeleton />
                  ) : (
          ...
          {section === "orders" ? (
            <div className="space-y-4">
              ...
              {ordersError ? (
                <div className="rounded-2xl border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
                  {ordersError}
                </div>
              ) : null}
              {ordersLoading ? (
