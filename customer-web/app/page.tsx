...
export default function HomePage() {
  ...
  const [categories, setCategories] = useState<Awaited<ReturnType<typeof getMenuPayload>>["categories"]>([]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        setProducts(payload.products.filter((p) => (p as { isAvailable?: boolean }).isAvailable !== false && p.available !== false));
        setCategories(payload.categories.filter((c) => c.isActive !== false));
      } catch (e) {
  ...
  return (
    <section className="space-y-10">
      ...
      {!online ? (
        <p
          className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100"
          role="status"
        >
          You appear offline — menu may not update until you reconnect.
        </p>
      ) : null}
