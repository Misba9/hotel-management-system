...
export default function MenuManagementPage() {
  ...
  async function addItem() {
    ...
    if (editingId) {
      await adminApiFetch(`/api/menu/${editingId}`, {
        method: "PATCH",
  ...
  async function toggleItem(item: { id: string; available: boolean }) {
    await adminApiFetch(`/api/menu/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteItem(id: string) {
    await adminApiFetch(`/api/menu/${id}`, { method: "DELETE" });
    if (editingId === id) {
      setEditingId(null);
      setName("");
      setPrice("");
  ...
  async function toggleCategory(category: { id: string; active?: boolean }) {
    await adminApiFetch(`/api/categories/${category.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteCategory(id: string) {
    await adminApiFetch(`/api/categories/${id}`, { method: "DELETE" });
    await loadCategories();
  }
