...
import { logServerInfo } from "@shared/utils/monitoring";
...
export async function enforceApiSecurity(request: Request, options: SecurityOptions = {}): Promise<SecuritySuccess | SecurityFailure> {
  if (options.rateLimit) {
    const limited = applyRateLimit(request, options.rateLimit);
    if (limited) {
      logServerInfo("Rate limit blocked", { endpoint: options.rateLimit.keyPrefix });
      return {
        ok: false,
        response: Response.json({ error: "Too many requests. Please try again shortly." }, { status: 429 })
      };
    }
  }
  ...
  if (!authHeader.startsWith("Bearer ")) {
    logServerInfo("Missing bearer token", { endpoint: options.roles.join(",") });
    return {
      ok: false,
  ...
  if (!token) {
    logServerInfo("Invalid bearer token", { endpoint: options.roles.join(",") });
    return {
      ok: false,
      response: Response.json({ error: "Invalid bearer token." }, { status: 401 })
    };
  }
  ...
  try {
    ...
    if (!options.roles.includes(role)) {
      logServerInfo("Permission denied", { role, requiredRoles: options.roles });
      return {
        ok: false,
